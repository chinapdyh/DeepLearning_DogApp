...
for i in range(len(human_files_short)):
    if(face_detector(human_files_short[i]) == True):
        human_correct_count = human_correct_count + 1

for i in range(len(dog_files_short)):
    if(face_detector(dog_files_short[i]) == True):
        dog_correct_count = dog_correct_count + 1        

human_correct_percent = human_correct_count / len(human_files_short)
dog_correct_percent = dog_correct_count / len(dog_files_short)
'''

def get_detector_percentage(detector, data_files):
    return np.mean([detector(f) for f in data_files])

human_correct_percent = get_detector_percentage(face_detector, human_files_short)
dog_correct_percent = get_detector_percentage(face_detector, dog_files_short)

print("Human correct percent is {}".format(human_correct_percent))
print("Dog correct percent is {}".format(dog_correct_percent))
...
# __回答:__
# 
# 实际使用中，用户的图片越清晰，干扰越好，自然结果越好。然而受制于外界环境条件如光照，天气，图片角度，相机抖动，图片保存时间等等情况，很难提供100%的清晰图片。
# 
# 当然，对于一些对精度有要求的场景，比如公安机关人脸检测，需要清晰度高的图片。
# 
# 我认为可以从两个方面着手：1.计算机视觉技术对不清晰的图片进行处理，如去噪，滤波等。2.在训练模型时，参入一些模糊的照片进行模型训练，这样就可以提高模型对于模糊照片的敏感度。
# 
...
human_correct_percent = get_detector_percentage(dog_detector, human_files_short)
dog_correct_percent = get_detector_percentage(dog_detector, dog_files_short)
...
# __回答:__ 
# 
# 1.首先，添加3个卷积层，size为2，过滤器的数量依次增大：16,32,64，这样增加了后面输出层的深度；为了保持通过卷积层的输出空间（W,H）不变，设置其strides为1，padding为same。激活函数设置为Relu，可以输出正值。input_shape设置为(224, 224, 3)，
# 在卷积层后分别添加3个最大池化层，size为2。这样就将卷积层的输出空间减少了一半，但没有改变深度。
# 卷积层和池化层将图片像素数组输入转换为所有空间信息都丢失、仅保留图片内容信息的数组。这样我们就可以只关心图片包含了哪一类信息。
# 
# 2.然后添加扁平化层。后面跟着两个密集层，解释图片中的内容。为了最后的输出为分类，激活函数设置为softmax。因为train_targets.shape 输出标记为133，所以设置softmax参数为133.这是通过查看train_tensors.shape得出的。
# 
# 3.添加dropout避免过拟合
...
# __回答:__ 
# 
# 对比上面的VGG16训练，添加了两个密集层，更好地描述图片中的内容。最后一层添加一个 softmax 激活函数，使其返回概率。
# 同时，为了避免过拟合，添加Dropout层。
# 
...
VGG19_model.add(Conv2D(filters=16, kernel_size=1, padding='same', activation='relu', 
                        input_shape=(7, 7, 512)))
...
VGG19_predict_breed(dog_files_short[1])


# In[151]:


VGG19_predict_breed(dog_files_short[5])
...
### TODO: 设计你的算法
### 自由地使用所需的代码单元数吧
def detect_dog(dog_image):
    if(dog_detector(dog_image) == True):
        return VGG19_predict_breed(dog_image)
    elif(face_detector(dog_image) == True):
        return "Is Human"
    else:
        print("Error image") 
...
# 2. 提出至少三点改进你的模型的想法。
# 
# 1.分辨出了是人或是狗，但我认为我传的是哈士奇图片，模型识别的是阿拉斯加，估计模型对长相较相近的图片无法很好地识别。
# 对于卡通图片无法很好地识别。
# 
# 2.改进模型的想法：
#     添加一些卡通图片进行训练。
#     精细划分不同类别的犬进行训练。
#     改用其他结构，激活函数，添加不同层改进CNN
...
'''
for i in range(5):
    print(detect_dog(dog_files_short[i]))
for i in range(5):
    print(detect_dog(human_files_short[i]))
'''
for i in range(3):
    #print(detect_dog(test_files[i]))
    pass

for i in range(3):
    #print(detect_dog(human_files[i]))
    pass

for i in range(6):
    print(detect_dog(('MyImage/{}.jpg').format(str(i + 1))))
